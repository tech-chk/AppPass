...
public class TreasureActivity extends Activity {
    ...
    private int minor3 = 0;
    private int minor4 = 0;
    private int minor5 = 0;

    //ここからBeaconで使用する変数、定数
    private Boolean isNews = false;
    //private MainActivity mainActivity;

    private static final String LOG_NAME = TreasureActivity.class.getSimpleName() + ".";
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        try {
                            ...
                            //取得した宝の画像をセット
                            String treasureurl1 = response.get("treasureurl1").toString();
                            if (treasureurl1 != null && treasureurl1 != "")
                            {
                                RequestQueue queue = Volley.newRequestQueue(myActivity);
                                NetworkImageView view = (NetworkImageView)findViewById(R.id.ivt1);
                                view.setImageUrl(imageurl + treasureurl1, new ImageLoader(queue, new LruCacheSample()));
                                view.setBackground(null);
                            }
                            String treasureurl2 = response.get("treasureurl2").toString();
                            if (treasureurl2 != null && treasureurl2 != "")
                            {
                                RequestQueue queue = Volley.newRequestQueue(myActivity);
                                NetworkImageView view = (NetworkImageView)findViewById(R.id.ivt2);
                                view.setImageUrl(imageurl + treasureurl2, new ImageLoader(queue, new LruCacheSample()));
                                view.setBackground(null);
                            }
                            String treasureurl3 = response.get("treasureurl3").toString();
                            if (treasureurl3 != null && treasureurl3 != "")
                            {
                                RequestQueue queue = Volley.newRequestQueue(myActivity);
                                NetworkImageView view = (NetworkImageView)findViewById(R.id.ivt3);
                                view.setImageUrl(imageurl + treasureurl3, new ImageLoader(queue, new LruCacheSample()));
                                view.setBackground(null);
                            }
                            String treasureurl4 = response.get("treasureurl4").toString();
                            if (treasureurl4 != null && treasureurl4 != "")
                            {
                                RequestQueue queue = Volley.newRequestQueue(myActivity);
                                NetworkImageView view = (NetworkImageView)findViewById(R.id.ivt4);
                                view.setImageUrl(imageurl + treasureurl4, new ImageLoader(queue, new LruCacheSample()));
                                view.setBackground(null);
                            }
                            String treasureurl5 = response.get("treasureurl5").toString();
                            if (treasureurl5 != null && treasureurl5 != "")
                            {
                                RequestQueue queue = Volley.newRequestQueue(myActivity);
                                NetworkImageView view = (NetworkImageView)findViewById(R.id.ivt5);
                                view.setImageUrl(imageurl + treasureurl5, new ImageLoader(queue, new LruCacheSample()));
                                view.setBackground(null);
                            }
        ...
        private void setViewRangingStatus(BeaconWatcher.RegionStateHolder h) {
            ...
            if (h != null && h.isRanging()) {
                ...
                if (beacons != null) {
                    ...
                    if ((beacon.getMajor() == 1 && beacon.getMinor() == 1 && minor1 != 1) ||
                            (beacon.getMajor() == 1 && beacon.getMinor() == 2 && minor1 != 1) ||
                            (beacon.getMajor() == 1 && beacon.getMinor() == 3 && minor1 != 1) ||
                            (beacon.getMajor() == 1 && beacon.getMinor() == 4 && minor1 != 1) ||
                            (beacon.getMajor() == 1 && beacon.getMinor() == 5 && minor1 != 1))
                    {
                        if (beacon.getProximity() == Beacon.Proximity.Immediate) //至近距離
                        {
                            ...
                            else if(beacon.getMinor() == 4)
                            {
                                minor4 = 1;
                            }
                            else if(beacon.getMinor() == 5)
                            {
                                minor5 = 1;
                            }

                            //距離
                            TextView txtKyori = (TextView)myActivity.findViewById(R.id.txtKyori);
                            txtKyori.setText("999");

                            //センサー画像
                            Bitmap bmp = null;
                            Resources r = myActivity.getResources();
                            bmp = BitmapFactory.decodeResource(r, R.drawable.senser0);
                            ImageView img = (ImageView)myActivity.findViewById(R.id.ivSenser);
                        ...
                        else if (beacon.getProximity() == Beacon.Proximity.Near) //近距離
                        {
                            //距離
                            TextView txtKyori = (TextView)myActivity.findViewById(R.id.txtKyori);
                            txtKyori.setText("1～5");

                            //センサー画像
                            Bitmap bmp = null;
                            Resources r = myActivity.getResources();
                            bmp = BitmapFactory.decodeResource(r, R.drawable.senser2);
                            ImageView img = (ImageView)myActivity.findViewById(R.id.ivSenser);
                            img.setImageBitmap(bmp);

                        }
                        else if (beacon.getProximity() == Beacon.Proximity.Far) // 遠距離
                        {
                            //距離
                            TextView txtKyori = (TextView)myActivity.findViewById(R.id.txtKyori);
                            txtKyori.setText("5～10");

                            //センサー画像
                            Bitmap bmp = null;
                            Resources r = myActivity.getResources();
                            bmp = BitmapFactory.decodeResource(r, R.drawable.senser1);
                            ImageView img = (ImageView)myActivity.findViewById(R.id.ivSenser);
                            img.setImageBitmap(bmp);
...
//                viewHolder.beacons.setVisibility(View.GONE);
        ...
        {
            //intentの設定
            Intent intent = new Intent(myActivity, TreasureActivity.class);
